    ...
    <div className="p-6 h-full grid grid-cols-12 gap-6 overflow-y-auto">
      ...
      <div className="col-span-12 md:col-span-4 lg:col-span-3">
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5">
          <div className="mb-4 flex justify-between items-center border-b border-slate-700 pb-2">
            <h3 className="text-blue-400 font-bold uppercase text-xs tracking-wider">Tunable Weights</h3>
            <span className={`text-xs font-bold text-white px-2 py-0.5 rounded ${weightColor}`}>
              {totalWeight}%
            </span>
          </div>
      ...
      <div className="col-span-12 md:col-span-8 lg:col-span-9 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
          <div className="bg-gradient-to-br from-blue-900/40 to-slate-800 border border-blue-500/50 rounded-lg p-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-3 opacity-10">
                <Info size={64} className="text-blue-200" />
            </div>
            <span className="text-[10px] uppercase text-blue-300 tracking-wider font-bold">Projected Winner</span>
            <div className="text-3xl font-bold text-white mt-1">{winner.name}</div>
            <div className="text-sm text-blue-200 mt-1 opacity-80">Score: {winner.finalScore}/10</div>
          </div>
          
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 flex flex-col justify-center">
            <span className="text-[10px] uppercase text-slate-400 tracking-wider font-bold">Key Strength</span>
            <div className="text-lg font-semibold text-white mt-1 line-clamp-2">{narrative.pros[0]}</div>
          </div>
          
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 flex flex-col justify-center">
            <span className="text-[10px] uppercase text-slate-400 tracking-wider font-bold">Best For</span>
